    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-purple-500/10 to-pink-600/10 rounded-xl p-6 border border-purple-500/30">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-1000 rounded-lg p-4">
            ...
            {monthlyStats.incomeTrend !== 0 && (
              <div className={`flex items-center gap-1 text-xs mt-1 ${monthlyStats.incomeTrend > 0 ? 'text-green-400' : 'text-rose-500'}`}>
          ...
          <div className="bg-slate-1000 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-rose-500">€{monthlyStats.currentExpenses.toFixed(0)}</p>
            {monthlyStats.expensesTrend !== 0 && (
              <div className={`flex items-center gap-1 text-xs mt-1 ${monthlyStats.expensesTrend > 0 ? 'text-rose-500' : 'text-green-400'}`}>
          ...
          <div className="bg-slate-1000 rounded-lg p-4">
            ...
            <p className={`text-2xl font-bold ${monthlyStats.currentBalance >= 0 ? 'text-indigo-500' : 'text-orange-500'}`}>
          ...
          <div className="bg-slate-1000 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-indigo-500">€{monthlyStats.avgDaily.toFixed(0)}</p>
            <p className="text-slate-400 text-xs mt-1">{monthlyStats.transactionCount} transazioni</p>
          </div>
        </div>
      </div>
      ...
      {categoryBreakdown.length > 0 && (
        <div className="bg-gradient-to-br from-indigo-400/10 to-indigo-50 rounded-xl p-6 border border-indigo-200">
          <div className="flex items-center gap-3 mb-4">
            <PieChart className="w-6 h-6 text-indigo-500" />
            <h3 className="text-lg font-semibold text-slate-800">Top 5 Categorie</h3>
          </div>
          ...
          <div className="space-y-3">
            {categoryBreakdown.map((cat, index) => (
              <motion.div
                ...
                className="bg-slate-1000 rounded-lg p-4"
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-slate-800 font-medium">{cat.category}</p>
                      <p className="text-slate-400 text-xs">{cat.count} transazioni</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-slate-800 font-bold">€{cat.total.toFixed(0)}</p>
