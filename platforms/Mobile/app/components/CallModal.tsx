...
export default function CallModal({ 
  ...
  // Auto-link: cerca match in rubrica quando cambia nome o telefono
  useEffect(() => {
    if (editCall || !clients.length) return
    const nameL = callerName.trim().toLowerCase()
    const phoneClean = phone.trim().replace(/\s/g, '')
    if (!nameL && !phoneClean) { setShowClientSuggestions(false); setMatchedClient(null); return }
    const matches = clients.filter(c => {
      const cNameL = c.name.toLowerCase()
      const cPhone = c.phone?.replace(/\s/g, '') || ''
      return (nameL.length >= 2 && cNameL.includes(nameL)) ||
             (phoneClean.length >= 3 && cPhone.includes(phoneClean))
    }).slice(0, 5)
    setShowClientSuggestions(matches.length > 0 && !matchedClient)
  }, [callerName, phone, clients, editCall, matchedClient])

  const fillFromClient = (client: any) => {
    setCallerName(client.name || callerName)
  ...
  const clientSuggestions = (!editCall && clients.length > 0 && !matchedClient) ? clients.filter(c => {
    const nameL = callerName.trim().toLowerCase()
    const phoneClean = phone.trim().replace(/\s/g, '')
    const cNameL = c.name.toLowerCase()
    const cPhone = c.phone?.replace(/\s/g, '') || ''
  ...
  useEffect(() => {
    if (editCall) {
      ...
      setMatchedClient(null); setShowClientSuggestions(false)
    }
  }, [editCall, isOpen])
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="relative">
                      ...
                      <input type="text" value={callerName} onChange={(e) => { setCallerName(e.target.value); setMatchedClient(null) }} required className={inputClass} placeholder="Mario Rossi" />
                      {/* Client suggestions dropdown */}
                      {clientSuggestions.length > 0 && showClientSuggestions && !editCall && (
                        <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl border border-slate-200/60 shadow-xl z-30 py-1 max-h-48 overflow-y-auto">
                          <div className="px-3 py-1.5 text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Clienti in rubrica</div>
                          {clientSuggestions.map(c => (
                            <button key={c.id} type="button" onClick={() => fillFromClient(c)}
                              className="w-full px-3 py-2 text-left flex items-center gap-2.5 hover:bg-indigo-50 transition-colors">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-[10px] text-slate-400 truncate">{[c.company, c.phone].filter(Boolean).join(' \u2022 ')}</p>
                              </div>
                            </button>
                          ))}
                        </div>
                      ...
                      {matchedClient && (
                        <div className="mt-1 flex items-center gap-1.5 px-2 py-1 bg-emerald-50 border border-emerald-200/60 rounded-lg">
                          ...
                          <button type="button" onClick={() => setMatchedClient(null)} className="ml-auto text-slate-400 hover:text-red-400 transition-colors">
                            <X className="w-3 h-3" />
                          </button>
                        </div>
                      )}
