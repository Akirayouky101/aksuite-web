...
export default function InfrastructurePanel({ isOpen, onClose }: Props) {
  ...
  const toggleVis = (id: string, _set: Set<string>, setter: React.Dispatch<React.SetStateAction<Set<string>>>) => {
    ...
    <>
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-2xl w-full max-w-4xl flex flex-col shadow-2xl overflow-hidden"
              ...
              <div className="px-5 py-4 border-b border-slate-100 shrink-0">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-base font-bold text-slate-800 leading-none">Infrastruttura Aziendale</h2>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Cerca per nome, IP, hostname…"
                    className="w-full bg-slate-100 rounded-xl pl-8 pr-8 py-2 text-sm text-slate-700 placeholder-slate-400 focus:outline-none"
                  />
                  {searchQuery && (
                    <button onClick={() => setSearchQuery('')} aria-label="Cancella ricerca" className="absolute right-3 top-1/2 -translate-y-1/2">
                ...
                {/* Type filter */}
                <div className="flex gap-1.5 mt-2 overflow-x-auto pb-0.5 [scrollbar-width:none]">
                    ...
                    {filtered.map(item => {
                      ...
                      return (
                        <button
                          ...
                          className="group text-left bg-white border border-slate-200 rounded-xl p-3.5 hover:border-slate-300 hover:shadow-md transition-all"
                        >
                          ...
                          <span className={`inline-block mt-2 px-1.5 py-0.5 rounded-md bg-gradient-to-r ${cfg.gradient} text-white text-[10px] font-semibold`}>
                            {cfg.label}
                          </span>
                        </button>
                      )
      ...
      <AnimatePresence>
        {detailItem && (() => {
          ...
          const cfg  = TYPE_CONFIG[item.type]
          interface FieldDef { label: string; value: string; id: string; mono: boolean }
          const fields: FieldDef[] = ([
            item.ip_address    ? { label: 'IP',        value: item.ip_address,    id: `d-${item.id}-ip`,   mono: true  } : null,
            item.hostname      ? { label: 'Hostname',  value: item.hostname,       id: `d-${item.id}-host`, mono: true  } : null,
            item.domain        ? { label: 'Dominio',   value: item.domain,         id: `d-${item.id}-dom`,  mono: true  } : null,
            item.mac_address   ? { label: 'MAC',       value: item.mac_address,    id: `d-${item.id}-mac`,  mono: true  } : null,
            item.port          ? { label: 'Porta',     value: item.port,           id: `d-${item.id}-port`, mono: true  } : null,
            item.os_version    ? { label: 'OS',        value: item.os_version,     id: `d-${item.id}-os`,   mono: false } : null,
            item.serial_number ? { label: 'S/N',       value: item.serial_number,  id: `d-${item.id}-sn`,   mono: true  } : null,
            item.location      ? { label: 'Posizione', value: item.location,       id: `d-${item.id}-loc`,  mono: false } : null,
          ] as (FieldDef | null)[]).filter((x): x is FieldDef => x !== null)
          ...
          return (
            ...
            >
              ...
              >
                {/* Header */}
                <div className={`bg-gradient-to-br ${cfg.gradient} px-5 py-5 shrink-0`}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          {item.isFavorite && <Star className="w-3.5 h-3.5 text-white/80 fill-white/80" />}
                          <h3 className="text-base font-bold text-white">{item.name}</h3>
                        </div>
                        <p className="text-xs text-white/70 mt-0.5">{cfg.label}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => setDetailItem(null)}
                ...
                <div className="overflow-y-auto flex-1 p-5 space-y-4">
                  ...
                  {(item.username || item.password || item.secondary_username || item.secondary_password) && (
                    <div className="bg-slate-50 rounded-xl p-3 space-y-2">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Credenziali</p>
                      {item.username && (
                        <div className="flex items-center gap-2">
                          ...
                          <CopyBtn text={item.username} id={`d-${item.id}-user`} copiedId={copiedId} onCopy={copy} />
                        </div>
                      )}
                      {item.password && (
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => toggleVis(item.id, visiblePwd, setVisiblePwd)} className="p-1 hover:bg-slate-200 rounded transition-colors">
                            {visiblePwd.has(item.id) ? <EyeOff className="w-3 h-3 text-slate-400" /> : <Eye className="w-3 h-3 text-slate-400" />}
                          </button>
                          <CopyBtn text={item.password} id={`d-${item.id}-pwd`} copiedId={copiedId} onCopy={copy} />
                        </div>
                      )}
                      {item.secondary_username && (
                        <div className="flex items-center gap-2">
                          ...
                          <CopyBtn text={item.secondary_username} id={`d-${item.id}-user2`} copiedId={copiedId} onCopy={copy} />
                        </div>
                      )}
                      {item.secondary_password && (
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => toggleVis(item.id, visiblePwd2, setVisiblePwd2)} className="p-1 hover:bg-slate-200 rounded transition-colors">
                            {visiblePwd2.has(item.id) ? <EyeOff className="w-3 h-3 text-slate-400" /> : <Eye className="w-3 h-3 text-slate-400" />}
                          </button>
                          <CopyBtn text={item.secondary_password} id={`d-${item.id}-pwd2`} copiedId={copiedId} onCopy={copy} />
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {item.notes && (
                    <p className="text-xs text-slate-500 italic px-1">{item.notes}</p>
                  )}
                </div>
                ...
                <div className="px-5 py-4 border-t border-slate-100 flex items-center gap-2 shrink-0">
                  ...
                  >
                    ...
                    {item.isFavorite ? 'Preferito' : 'Aggiungi ai preferiti'}
                  </button>
                  <button
                    onClick={() => openEdit(item)}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100 transition-colors"
                  >
                    <Pencil className="w-3.5 h-3.5" /> Modifica
                  </button>
                  <button
                    onClick={() => setDeleteConfirm(item.id)}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border bg-white text-red-500 border-red-200 hover:bg-red-50 transition-colors ml-auto"
                  >
          ...
          >
            ...
            >
              ...
              <div className="p-5 overflow-y-auto space-y-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Credenziali principali</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className={`${inp} pr-9`}
                        />
                        <button type="button" aria-label="Mostra/nascondi password" onClick={() => setShowFormPwd(v => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600">
                ...
                {showDual && (
                  <div>
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Credenziali secondarie</p>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <input
                            ...
                            className={`${inp} pr-9`}
                          />
                          <button type="button" aria-label="Mostra/nascondi password 2" onClick={() => setShowFormPwd2(v => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.isFavorite} onChange={e => f('isFavorite', e.target.checked)} aria-label="Aggiungi ai preferiti" className="accent-amber-400 w-4 h-4" />
        ...
        {deleteConfirm && (
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-slate-500 mb-5">Questa operazione non si può annullare.</p>
