          ...
          <div className="relative bg-white/90 backdrop-blur-2xl rounded-2xl max-h-[90vh] overflow-hidden border border-slate-200/60 shadow-2xl shadow-slate-200/50">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200/60 bg-white/60 flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800">Limiti Budget</h2>
                  <p className="text-xs text-slate-400 mt-0.5">{limits.length} limiti configurati</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="p-6 overflow-y-auto overflow-x-hidden max-h-[calc(90vh-88px)]">
              ...
              {limits.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-400 text-sm mt-2">Imposta limiti per controllare le tue spese!</p>
                </div>
              ) : (
                        ...
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div className="flex items-start gap-3 flex-1">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <h3 className="text-slate-800 font-bold text-lg">{limit.category}</h3>
                              ...
                              <div className="relative">
                                <div className="w-full bg-slate-50 rounded-full h-3 mb-2">
                                  <motion.div
                                    initial={{ width: 0 }}
                                ...
                                <div className="flex items-center justify-between text-sm">
                                  <span className="text-slate-400">
                                    Speso: <span className="font-bold text-slate-800">€{limit.current_spending.toFixed(2)}</span>
                                  </span>
                                ...
                                {limit.status !== 'exceeded' && (
                                  <div className="text-slate-400 text-xs mt-1">
                                    Disponibile: <span className="text-slate-800 font-semibold">€{limit.remaining_amount.toFixed(2)}</span>
                                  </div>
                                )}
              ...
              {limits.length > 0 && (
                <div className="mt-6 bg-indigo-50 border border-indigo-200/60 rounded-xl p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-indigo-400 font-medium mb-1">Suggerimento</p>
