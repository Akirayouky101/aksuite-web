...
export default function PasswordsWorkspace({ passwords, onNew, onEdit, onDetail, onDelete, categories = [], onCreateCategory, onUpdateCategory, onDeleteCategory }: PasswordsWorkspaceProps) {
  ...
  return (
    <section className="ak-workspace ak-passwords-workspace">
      ...
      {folders.length > 0 && <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{folders.map(path => { const name = path.split(' / ').pop() || path; const category = categories.find(item => getCategoryPath(item.id) === path); return <div key={path} className="ak-bento flex items-center gap-3"><button onClick={() => setSelectedPath(path.split(' / '))} className="flex min-w-0 flex-1 items-center gap-3 text-left"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#ddd7ff] text-[#4b3ba5]"><Folder className="h-5 w-5" /></span><span className="min-w-0"><strong className="block truncate text-[#2d2754]">{name}</strong><small className="text-slate-400">{passwords.filter(item => String(item.category || '').startsWith(path)).length} credenziali</small></span></button>{category && <button onClick={() => setEditingCategory(category)} title={`Modifica ${name}`} className="rounded-lg p-2 text-slate-400 hover:bg-indigo-50 hover:text-indigo-600"><Pencil className="h-4 w-4" /></button>}<button onClick={() => setCategoryParentId(getCategoryIdByPath(path))} title={`Crea sottocategoria in ${name}`} className="rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1 text-lg leading-none text-indigo-600 transition hover:bg-indigo-100">+</button><ChevronRight className="h-4 w-4 shrink-0 text-slate-400" /></div> })}</div>}
      ...
      <PasswordCategoryModal
        ...
        parentName={editingCategory?.parent_id ? getCategoryPath(editingCategory.parent_id) : undefined}
        onClose={() => setEditingCategory(null)}
        onSave={async (name) => { if (editingCategory) await onUpdateCategory?.(editingCategory.id, name); setEditingCategory(null) }}
        onDelete={async () => { if (editingCategory && window.confirm(`Eliminare ${editingCategory.name}? Le credenziali resteranno salvate senza categoria.`)) { await onDeleteCategory?.(editingCategory.id); setEditingCategory(null) } }}
      />
    </section>
  )
}
