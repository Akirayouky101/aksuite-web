...
export default function NotesStickyWidget({ notes, onOpenNote, onAddNote }: NotesStickyWidgetProps) {
  const featured = [...notes].filter(note => note.is_pinned)
    .sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime())
    .slice(0, 3)
  const fallback = featured.length === 0
    ? [...notes].sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()).slice(0, 3)
    : featured
  ...
  const prev = () => setIndex(i => (i - 1 + stack.length) % stack.length)
  const next = () => setIndex(i => (i + 1) % stack.length)

  return (
    <div className="relative flex min-h-48 flex-col justify-between rounded-[1.75rem] p-4">
      {stack.slice(0, 3).map((note, i) => {
        if (i === index) return null
        const depth = i > index ? i - index : i - index + stack.length
        return (
          <div
            key={note.id}
